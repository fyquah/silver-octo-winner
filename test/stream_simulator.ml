...
let send_packet ?(config = default_config) (h @ local) { Send_ports.source; ready } (packet : string) =
  ...
  let tkeep_last =
    ...
    Bits.of_unsigned_int ~width:4
      (if l = 0 then 0xF else (1 lsl l) - 1)
  in
  for i = 0 to num_words - 1 do
    ...
    source.value.tdata  := Bits.of_unsigned_int ~width:32 ((ch3 lsl 24) + (ch2 lsl 16) + (ch1 lsl 8) + ch0);
...
let receive_packet ?(config = default_config) (h @ local) (ports : Receive_ports.t) =
  ...
  let rec loop () =
    ...
    if not (Bits.to_bool stream.value.tlast) then 
      loop ()
  in
  loop ();
