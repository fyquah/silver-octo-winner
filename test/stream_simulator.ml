...
let send_packet ?(config = default_config) (h @ local)
    { Send_ports.source; ready } (packet : string) =
  let num_words = (String.length packet + 3) / 4 in
  let get x = if x < String.length packet then Char.to_int packet.[x] else 0 in
  let tkeep_last =
    let l = String.length packet mod 4 in
    Bits.of_unsigned_int ~width:4 (if l = 0 then 0xF else (1 lsl l) - 1)
  in
  for i = 0 to num_words - 1 do
    let ch3 = get ((i * 4) + 3) in
    let ch2 = get ((i * 4) + 2) in
    let ch1 = get ((i * 4) + 1) in
    let ch0 = get ((i * 4) + 0) in
    while not (sample_from_config config) do
      Step.cycle h ()
    done;
    source.valid := Bits.vdd;
    source.value.tdata :=
      Bits.of_unsigned_int ~width:32
        ((ch3 lsl 24) + (ch2 lsl 16) + (ch1 lsl 8) + ch0);
    source.value.tfirst := Bits.of_bool (i = 0);
    source.value.tlast := Bits.of_bool (i = num_words - 1);
    source.value.tkeep := if i = num_words - 1 then tkeep_last else Bits.ones 4;
    Step.cycle h ();
    while not (Bits.to_bool !(ready.before_edge)) do
      Step.cycle h ()
    done;
    source.valid := Bits.gnd
  done

let receive_packet ?(config = default_config) (h @ local)
    (ports : Receive_ports.t) =
  ...
  let rec loop () =
    ...
    let stream = Stream.With_valid.map ~f:( ! ) ports.source.before_edge in
    assert (Bool.equal !is_first_word (Bits.to_bool stream.valid));
    let this_word =
      let popcount = Bits.to_unsigned_int (Bits.popcount stream.value.tkeep) in
      Bits.sel_bottom ~width:(popcount * 8) stream.value.tdata
    in
    Vec.push_back collected this_word;
    if not (Bits.to_bool stream.value.tlast) then loop ()
  in
  loop ();
