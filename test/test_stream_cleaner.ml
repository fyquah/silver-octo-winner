...
let%expect_test "" =
  ...
  let outputs =
    {
      Before_and_after_edge.before_edge =
        Cyclesim.outputs ~clock_edge:Before simulator;
  ...
  Step.run_until_finished () ~simulator ~testbench:(fun h ->
      ...
      let task =
        Step.spawn h (fun h () ->
            Stream_simulator.receive_packet h
              {
                source =
                  Before_and_after_edge.map outputs ~f:(fun o -> o.dn_src);
                ready = inputs.dn_ready;
              })
      in
      Stream_simulator.send_packet h
        {
